...
class ShoulderCheckAdvancedGame(GameManager):
    ...
    def randomColors(self, game):
        ...
        # Color front targets randomly (2, 3, 4, 5, 7) and color one back target the same color (1, 7)
        for i in range(game.getRoom() * 8 + 2 - indicator, game.getRoom() * 8 + 8 - indicator):
            color = colors.pop(randint(0, len(colors) - 1))
            game.colorTarget(color, i)

            if i == game.getTarget():
                game.colorTarget(color, 1 + 6 * indicator)
